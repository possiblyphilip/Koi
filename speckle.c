...
//set my whole block to black
    for (from_row = 0; from_row < job_args->height; from_row++)
    {
    	...
    	{
	    job_args->array_out[from_col][from_row][0] = 0;
	    job_args->array_out[from_col][from_row][1] = 0;
	...
	for (from_col = job_args->start_colum; from_col < max_col; from_col++)
	{
	    ...
	    color_slider[counter].red = job_args->array_in[from_col][from_row][0];
	    color_slider[counter].green = job_args->array_in[from_col][from_row][1];
	    color_slider[counter].blue = job_args->array_in[from_col][from_row][2];
